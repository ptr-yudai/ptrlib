...
def _aes_mix_columns(s: List[List[int]]):
    for i in range(4):
        ...
        temp = s[i][0]
        s[i][0] ^= t ^ _aes_xtime(s[i][0] ^ s[i][1])
        s[i][1] ^= t ^ _aes_xtime(s[i][1] ^ s[i][2])
...
class Instructions:
    ...
    def aesenc(self, state: Union[str, bytes], round_key: Union[str, bytes]) -> bytes:
        ...
        Args:
            state (bytes): A 16-bytes state.
            round_key (bytes): A 16-bytes

        Returns:
            bytes: State bytes after AESENC is applied.

        Raises:
            ValueError: `state` or `round_key` is invalid.
        """
        if len(state) != 16:
        ...
        mstate = _aes_bytes_to_matrix(state)
        mkey = _aes_bytes_to_matrix(round_key)
        _aes_shift_rows(mstate)
        _aes_sub_bytes(mstate)
        _aes_mix_columns(mstate)
    ...
    def aesenc_inv(self, state: Union[str, bytes], round_key: Union[str, bytes]) -> bytes:
        ...
        Returns:
            bytes: State bytes after the inverse of AESENC is applied.
        ...
        mkey = _aes_bytes_to_matrix(round_key)
        _aes_add_round_key(mstate, mkey)
        _aes_inv_mix_columns(mstate)
        _aes_inv_sub_bytes(mstate)
