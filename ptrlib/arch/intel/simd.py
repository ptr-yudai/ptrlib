...
def _aes_bytes_to_matrix(data):
    if isinstance(data, str):
        data = str2bytes(data)
    return [list(data[i:i+4]) for i in range(0, len(data), 4)]

def _aes_matrix_to_bytes(matrix):
...
def _aes_sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = s_box[s[i][j]]

def _aes_inv_sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = inv_s_box[s[i][j]]

def _aes_add_round_key(s, k):
    for i in range(4):
        for j in range(4):
            s[i][j] ^= k[i][j]
...
def intel_aesenc(state, round_key):
    ...
    state = _aes_bytes_to_matrix(state)
